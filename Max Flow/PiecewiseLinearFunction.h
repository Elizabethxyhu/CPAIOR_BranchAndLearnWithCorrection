...
PiecewiseLinearFunction DivisionSpecial(PiecewiseLinearFunction fun1, double num) {
    PiecewiseLinearFunction ms;
    ms.assign(fun1.start_x[0], fun1.end_x[0], fun1.a[0] / num, fun1.b[0] / num, fun1.id[0] / num);
    return ms;
}
...
PiecewiseLinearFunction SupCombination(PiecewiseLinearFunction fun1, PiecewiseLinearFunction fun2) {
    ...
    int j = 0;
//    double cur_pos = -INF;
    double cur_pos = min(fun1.start_x[0], fun2.start_x[0]);
    PiecewiseLinearFunction comb_fun;
    while (i < fun1.start_x.size() || j < fun2.start_x.size()) {
        double x = (fun2.b[j] - fun1.b[i]) / (fun1.a[i] - fun2.a[j]);
        if (fun1.a[i] == fun2.a[j]){
            x = INF;
        }
