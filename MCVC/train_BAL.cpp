...
Result De_MCVC(MCVC G, int cnt, vector<int> selected, int haveToCover) {
	...
	if (cnt == G.nodeNum || haveToCover == 0) {
		...
		Result result1 = De_MCVC(G1, cnt, selected1, haveToCover1);
		Result result2 = De_MCVC(G2, cnt, selected2, haveToCover2);
		Result result;
...
PiecewiseLinearFunction CostTrain_MCVC(MCVC G, int cnt, vector<int> selected, int haveToCover) {
	...
	if (cnt == G.nodeNum || haveToCover == 0) {
		...
		PiecewiseLinearFunction result1 = CostTrain_MCVC(G1, cnt, selected1, haveToCover1);
		PiecewiseLinearFunction result2 = CostTrain_MCVC(G2, cnt, selected2, haveToCover2);
...
{
	if (argc != 10) {
		std::cerr << "usage: ./train_BAL [graph] [train edge data] [test edge data] [train cost data] [test cost data] [pre edge file] [pre cost file] [iteration] [runtime file]" << std::endl;
		exit(0);
	}
	...
	for (int T = 0; T < iteration; T++) {
		...
		for (; cntAB < featureNum; cntAB++) {
			...
			for (int benN = 0; benN < trainNum; benN++) {
				...
				vector<int> STrue;
				Result realValue = De_MCVC(GTrue, -1, STrue, HTCTrue);
				STrue.clear();
				...
				for (int i = 0; i < transPoint.size(); i++) {
					...
					MCVC G(edgeNum, nodeNum, preCost, costTemp, edge);
					int HTC = edgeNum - 1;
					vector<int> S;
					Result weight = De_MCVC(G, -1, S, HTC);
					double preValue = CostTest_MCVC(weight.finalSelected, realCost);
...
//			std::cout << "minimum regret = " << alphaRegret << endl;
		...
		for (; cntAB < 2 * featureNum; cntAB++) {
			...
			for (int benN = 0; benN < trainNum; benN++) {
				...
				MCVC GTrue(edgeNum, nodeNum, trainCost, cost, realEdge);
				int HTCtrue = edgeNum - 1;
				vector<int> S;
				Result realValue = De_MCVC(GTrue, -1, S, HTCtrue);

				S.clear();
				MCVC Gpre(edgeNum, nodeNum, trainCost, cost, preEdge);
				int HTCpre = edgeNum - 1;
				//PiecewiseLinearFunction predictValue = CorrectionalCostTrain_MCVC(Gpre, -1, S, delSrc, delDes, useCorrectional, HTCpre);
				PiecewiseLinearFunction predictValue = CostTrain_MCVC(Gpre, -1, S, HTCpre);
